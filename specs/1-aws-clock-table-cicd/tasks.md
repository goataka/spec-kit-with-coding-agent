...
- [ ] 前提条件を文書化（AWSアカウント、初回手動OIDC設定、CDK管理への移行）
- [ ] 初期セットアップ手順を文書化（手動OIDC作成 → CDKデプロイ → 手動設定削除）
- [ ] デプロイプロセスを文書化
...
1. タスク 1.1 → 1.2 → 1.2a → 1.3 → 1.4 (基盤 + OIDC管理)
2. タスク 2.1 → 2.2 (CI/CD)
3. タスク 3.1 → 3.2 (テスト)
...
- デプロイに手動承認は不要（完全自動化）
- **OIDC管理**: 初回は手動でOIDCとIAMロールを設定し、CDKデプロイ後はCDK管理に移行。手動設定は削除する
