...
3. **前提** 初回は手動でOIDCを設定, **実行** CDKでOIDCとロールをデプロイ, **結果** 手動設定を削除し、以降はCDK管理のOIDCを使用

---

### ユーザーストーリー 3 - CloudFormation テンプレートの出力 (優先度: P1)

システム管理者として、CDK synthを手動ワークフローで実行してCloudFormationテンプレートを出力したい。これにより、初回のみ手動でOIDCとIAMロールを設定するためのテンプレートを準備できる。

**この優先度の理由**: 初回のブートストラップに必要。synthで生成したテンプレートを使用してOIDCとIAMロールを手動作成し、その後CDKデプロイで自動管理に移行する。
...
- **FR-008b**: 初回セットアップは手動でOIDCを設定し、CDKデプロイ後はCDK管理のOIDCを使用し、手動設定を削除しなければならない
...
- AWSアカウントが存在する
- 初回セットアップ時に、手動でOIDCプロバイダーとIAMロールを設定する権限がある（その後CDKで管理）
...
- CDK管理によるOIDC認証（初回のみ手動セットアップ、以降はCDK管理）
